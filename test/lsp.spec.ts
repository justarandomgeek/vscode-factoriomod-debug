...
import { CodeAction, CodeActionKind, CodeActionParams, CodeActionRequest, DidChangeTextDocumentNotification, DidChangeTextDocumentParams, DidCloseTextDocumentNotification, DidCloseTextDocumentParams, DocumentSymbol, DocumentSymbolParams, DocumentSymbolRequest, ProtocolNotificationType, PublishDiagnosticsParams, SymbolKind } from "vscode-languageserver-protocol";
	...
	suite("Changelog", ()=>{
		...
		setup(async function() {
			...
			await clientConnection.sendNotification(DidOpenTextDocumentNotification.type,
				{ textDocument: docItem() } as DidOpenTextDocumentParams);
		});

		teardown(async function() {
			await clientConnection.sendNotification(DidCloseTextDocumentNotification.type,
				{ textDocument: docItem() } as DidCloseTextDocumentParams);
		...
		async function singleCodeActionShouldFix(diags:PublishDiagnosticsParams) {
			const actions = await clientConnection.sendRequest(CodeActionRequest.type, {
				textDocument: docItem(),
		...
		test("separator-length", async function() {
			...
			await singleCodeActionShouldFix(diags);
		});
		...
		test("symbols", async function() {
			...
			const symbols = <DocumentSymbol[]> await clientConnection.sendRequest(DocumentSymbolRequest.type, { textDocument: docItem() } as DocumentSymbolParams);
