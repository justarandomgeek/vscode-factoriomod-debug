...
function saveString(str:string): Buffer {
	if (!str) {
...
function typeTag(type:PropertyTreeType): Buffer {
	return Buffer.from([type, 0 /* isAnyType */ ]);
}

export function savePTree(tree:PropertyTreeData):Buffer {
	switch (tree.type) {
		case PropertyTreeType.string:
		{
			return Buffer.concat([
				typeTag(PropertyTreeType.string),
				saveString(tree.value),
			]);
		}
		case PropertyTreeType.bool:
			return Buffer.concat([
				typeTag(PropertyTreeType.bool),
		...
		case PropertyTreeType.number:
		{
			...
			return Buffer.concat([
				typeTag(PropertyTreeType.number),
				b,
			]);
		}
		case PropertyTreeType.signedinteger:
		{
			...
			return Buffer.concat([
				typeTag(PropertyTreeType.signedinteger),
				b,
			]);
		}
		case PropertyTreeType.unsignedinteger:
		{
			...
			return Buffer.concat([
				typeTag(PropertyTreeType.unsignedinteger),
				b,
			]);
		}
		case PropertyTreeType.none:
		{
			return typeTag(PropertyTreeType.none);
		}
		case PropertyTreeType.list:
		{
			...
			return Buffer.concat([
				typeTag(PropertyTreeType.list),
				size,
				Buffer.concat(tree.value.map((v)=>{
					return Buffer.concat([
						saveString(""), // no key for list
						savePTree(v),
					]);
				})),
			]);
		...
		case PropertyTreeType.dictionary:
		{
			...
			return Buffer.concat([
				typeTag(PropertyTreeType.dictionary),
				size,
				Buffer.concat(buffs),
			]);
		}
