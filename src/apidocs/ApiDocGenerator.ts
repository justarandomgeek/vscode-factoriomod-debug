...
export class ApiDocGenerator {
	...
	public generate_sumneko_docs() {
		...
		ms.write(`\n`);
		this.generate_sumneko_builtin(ms);
		ms.write(`\n`);
		this.generate_sumneko_defines(ms);
		ms.write(`\n`);
		this.generate_sumneko_events(ms);
		ms.write(`\n`);
		this.generate_sumneko_classes(ms);
		ms.write(`\n`);
		this.generate_sumneko_concepts(ms);
		ms.write(`\n`);
		this.generate_sumneko_custom(ms);
		ms.write(`\n`);
		this.generate_sumneko_table_types(ms);
		ms.write(`\n`);
		return ms.toBuffer();
	}
	...
	private generate_sumneko_classes(output:WritableMemoryStream) {
		this.docs.classes.forEach(aclass=>this.add_sumneko_class(output,aclass));
